const data = [
  ...
  const loadCar = (cars) => {
    const carWrap = document.getElementById('personWrap');
    cars.forEach(car => {
        console.log(car);
        ...
        <div class="card">
        <div class="card-body">
        <img width="100%" src="${car.imageURL}" />
            <h5 class="card-title">Car Name: ${car.name}</h5>
          <p>Car detail: ${car.description}</p>
          <span class="btn btn-success">Car price: ${car.price}</span>
        </div>
      </div>
        `
        carWrap.appendChild(createEle);
